...
export const PageBox: FC<PageBoxProps> = ({description, title, className, selected, onClick, children, to, variant}) => {
    ...
    return <div
        className={cn(
            "relative rounded-xl border border-gray-200 bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)]",
            variant === "gray" && "bg-gray-50",
            isClickable && "cursor-pointer transition hover:-translate-y-[1px] hover:border-cyan-200 hover:shadow-[0_14px_28px_rgba(8,47,73,0.08)]",
            className
        )}
        onClick={handleClick}
    >
        ...
        {to && <ChevronRight className="absolute right-4 top-4 text-gray-400"/>}
